...
# НИКАКИХ ПРОКСИ! ЧИСТОЕ СОЕДИНЕНИЕ
...
def get_player_stats(aid):
    try:
        ...
        r = requests.get(url, params={'application_id': WG_APP_ID, 'account_id': aid}, headers=HEADERS,
                         verify=False).json()
        if r.get('data'):
            d = r['data'][str(aid)]
            s = d['statistics']['all']
...
def get_tank_marks(aid):
    try:
        ...
        r_enc = requests.get(url_enc, params={'application_id': WG_APP_ID, 'tank_id': ",".join(tank_ids),
                                              'fields': 'short_name'}, headers=HEADERS, verify=False).json()
...
async def start(msg: types.Message):
    ...
    await msg.answer("👋 <b>Привет!</b>\nНапиши <code>/reg ТвойНик</code> для регистрации.", reply_markup=kb_main(),
...
async def my_marks(msg: types.Message):
    ...
    if user_id not in db:
        ...
        return
    saved_nick = db[user_id]
    loading = await msg.answer(f"🚜 Анализ ангара <b>{saved_nick}</b>...", parse_mode=ParseMode.HTML)
    aid = get_account_id(saved_nick)
    if aid:
